...
`labels/unified_wt.example.csv` **不是数据集，不能直接训练**。它是零数据、仅含列名的 schema 模板，公开了 `unified_wt.csv` 所需的数据契约，防止协作者误填列顺序。复制为 `labels/unified_wt.csv` 后，只能填写有授权、可追溯的样品标识、数据来源、核验状态和元素质量分数。详见 [labels/README.md](labels/README.md)。真实标签必须留在本地并保持 Git 忽略。

NASA ChemCam 产品应从 [NASA PDS](https://pds.nasa.gov/) 获取，并遵守集合的产品说明、引用和再分发条款。NIST ASD 是谱线元数据来源，不是带化验标签的 LIBS 训练集。

### 7. 数据公开范围与隐私边界

本公开仓库刻意只包含源代码、环境定义、零数据标签模板和项目文档。下列内容未公开：
...
`.gitignore` 对本地数据目录、`labels/unified_wt.csv`、权重和常见输出格式设置了排除规则。提交前仍应人工检查 `git status` 与待推送内容；忽略规则不能替代数据所有者的授权判断。

### 8. 在现有条件下的下一步

优先级不应是继续堆叠大模型，而应是让问题可识别、可复现：

1. **P0：固定实验合同。** 将存在性检测与连续 wt% 回归完全分开；折内完成所有数据驱动选择；逐元素发布样本数、浓度范围、谱线覆盖、SNR、饱和率和均值基线误差。
2. **P1：盘点已有真值。** 把已测但尚未进入统一标签表的、可追溯化验结果进行样品 ID 审计后纳入；这不是合成数据扩充。
3. **P2：先做小模型。** 在原生谱段上比较线强比、Ridge/Elastic Net、PLS/interval-PLS 和带不确定度的 Gaussian process/kernel Ridge；任何迁移映射必须仅由训练折拟合。
4. **P3：补齐物理样本。** 采用标准化压片制样，采集 30--50 个基体匹配且覆盖连续浓度梯度的独立标样，并预留从未参与模型选择的盲测集。
5. **P4：再评价深度迁移。** 只有在小模型在嵌套 LOSO 下稳定优于基线、且独立样本量足够后，才考虑冻结后微调的双视图编码器或其他深度模型。
